...
classdef test_KPIWindow_Consumer < matlab.uitest.TestCase% subclass of matlab.uitest.TestCase
    properties
        MainApp                                 % will hold the GUI Framework
        ValueChains                             % entries to the Value chain drop down menue
        Window_Name = 'KPIWindow_Consumer';     % the KPI Window for which these tests are relevant
    ...
    methods (Test)
        function open_and_close(testCase)
            ...
            open_close_KPI_Window(testCase, testCase.Window_Name, testCase.MainApp.Button_Consumer);
